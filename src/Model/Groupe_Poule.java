...
//	}
	...
	public List<Team> get_end_winner(){
		this.last_setting_winners();
		List<Team> teamtab = this.getCadeau_roman_lovelove();
		return teamtab;
	}
	...
	public void deroulement_round(Poule poule_selec){
		...
		while(parcours1 < teamtab.size()-1){
			while(parcours2 < teamtab.size()){
				view_type.display_versus(teamtab.get(parcours1).get_name(),teamtab.get(parcours2).get_name());
				//saisie des score par team 
				//
				score1 =view_type.fill_in_score(teamtab.get(parcours1).get_name());
				
		
				score2 =view_type.fill_in_score(teamtab.get(parcours2).get_name());
				//
				// affichage score
				//
				view_type.display_score(teamtab.get(parcours1).get_name(), teamtab.get(parcours2).get_name(), score1, score2);
	...
	public void setCadeau_roman_lovelove(List<Team> cadeau_roman_lovelove) {
		this.cadeau_roman_lovelove.removeAll(cadeau_roman_lovelove);
